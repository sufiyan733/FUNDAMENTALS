...
//   return (
//     <>
//       <div ref={dot} style={{ position:"fixed",top:0,left:0,zIndex:99999,pointerEvents:"none",width:8,height:8,borderRadius:"50%",background:"#00f7ff",mixBlendMode:"difference",willChange:"transform" }} />
//       <div ref={ring} style={{ position:"fixed",top:0,left:0,zIndex:99998,pointerEvents:"none",width:40,height:40,borderRadius:"50%",border:"1px solid rgba(0,247,255,0.4)",willChange:"transform",transition:"width .38s cubic-bezier(.34,1.56,.64,1),height .38s cubic-bezier(.34,1.56,.64,1)" }} />
//     </>
//   );
// }

// // ─── BOOT (UNCHANGED — PERFECT) ───────────────────────────────────────────────
...
//   }, []);
//   return (
//     <div ref={ref} style={{ position:"fixed",top:0,left:0,right:0,zIndex:100,display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 32px",background:"rgba(5,6,14,.92)",borderBottom:"1px solid #1a2030",backdropFilter:"blur(20px)",fontFamily:"'Fira Code',monospace",fontSize:10,letterSpacing:2 }}>
...
//   }, []);
//   return (
//     <div style={{ position:"fixed",top:41,left:0,right:0,height:2,zIndex:101,background:"rgba(255,255,255,0.06)" }}>
//       <div ref={ref} style={{ position:"absolute",inset:0,background:"linear-gradient(90deg,#00f7ff,#00cfff)",transformOrigin:"left",transform:"scaleX(0)",boxShadow:"0 0 16px 2px #00f7ff, 0 0 4px #fff" }}/>
//     </div>
//   );
// }
...
//   return (
//     <div style={{ borderTop:"1px solid #1a2030",borderBottom:"1px solid #1a2030",padding:"9px 0",overflow:"hidden",position:"relative",marginBottom:52 }}>
//       <div style={{ position:"absolute",left:0,top:0,bottom:0,width:80,zIndex:2,background:"linear-gradient(90deg,#05060e,transparent)",pointerEvents:"none" }}/>
//       <div style={{ position:"absolute",right:0,top:0,bottom:0,width:80,zIndex:2,background:"linear-gradient(270deg,#05060e,transparent)",pointerEvents:"none" }}/>
//       <div style={{ display:"flex",animation:"tk 24s linear infinite",whiteSpace:"nowrap",width:"max-content" }}>
//         {items.map((item,i)=>(
//           <span key={i} style={{ fontSize:9,letterSpacing:4,color:"#253040",fontFamily:"'Fira Code',monospace",paddingRight:48,display:"inline-flex",alignItems:"center" }}>
//             <span style={{ color:"#00f7ff18",marginRight:18,fontSize:7 }}>*</span>{item}
//           </span>
//         ))}
//       </div>
...
//   return (
//     <div ref={wrapRef} style={{ overflow:"hidden",borderTop:"1px solid #1e2a3a",borderBottom:"1px solid #1e2a3a",padding:"10px 0",marginBottom:2,position:"relative" }}>
...
//         {words.map((w, i) => (
//           <span key={i} style={{
//             fontSize:"clamp(52px,7.5vw,100px)", fontWeight:900, letterSpacing:-3,
//             fontFamily:"'Fira Code',monospace", paddingRight:64,
//             color: i % 2 === 0 ? "transparent" : dim,
//             WebkitTextStroke: i % 2 === 0 ? `1px ${accent}55` : "none",
//             textShadow: i % 2 !== 0 ? "none" : `0 0 80px ${accent}08`,
//             transition: "color .3s",
//           }}>{w}</span>
//         ))}
//       </div>
//       {/* subtle glow line at bottom */}
//       <div style={{ position:"absolute",bottom:0,left:0,right:0,height:1,background:`linear-gradient(90deg,transparent,${accent}33,transparent)`,pointerEvents:"none" }}/>
//     </div>
//   );
// }
...
//       r: Math.random()*1.2+.3, o: Math.random()*.22+.04,
//     }));
...
//         ctx.fillStyle=`rgba(0,247,255,${p.o})`; ctx.fill();
//       });
//       for(let i=0;i<pts.length;i++) for(let j=i+1;j<pts.length;j++){
//         const dx=pts[i].x-pts[j].x, dy=pts[i].y-pts[j].y, d=Math.sqrt(dx*dx+dy*dy);
//         if(d<85){
//           ctx.beginPath(); ctx.moveTo(pts[i].x,pts[i].y); ctx.lineTo(pts[j].x,pts[j].y);
//           ctx.strokeStyle=`rgba(0,247,255,${(1-d/85)*.04})`; ctx.lineWidth=0.5; ctx.stroke();
//         }
//       }
...
//       background:`radial-gradient(circle at ${mousePos.x*100}% ${mousePos.y*100}%, ${lang.accent}30, transparent 70%)`,
...
//         border:`1px solid ${hov?lang.accent+"99":"#1a2030"}`,
//         background:hov?`rgba(${rgb},.04)`:"rgba(5,7,14,.95)",
//         padding:"clamp(28px,5vw,48px) clamp(22px,4vw,38px)",
...
//         background:`radial-gradient(ellipse at ${mousePos.x*100}% ${mousePos.y*100}%, rgba(${rgb},0.12) 0%, transparent 65%)`,
//         transition:"background .12s",
//       }}/>

//       {/* top glow */}
//       <div style={{ position:"absolute",top:0,left:0,right:0,height:200,pointerEvents:"none",background:`radial-gradient(ellipse at 50% 0%,rgba(${rgb},${hov?.14:0}) 0%,transparent 70%)`,transition:"background .5s" }}/>

//       {/* top scan line */}
//       <div ref={topLine} style={{ position:"absolute",top:0,left:0,right:0,height:2,background:`linear-gradient(90deg,transparent,${lang.accent},transparent)`,boxShadow:`0 0 24px 2px ${lang.accent}, 0 0 8px ${lang.accent}`,transformOrigin:"center",transform:"scaleX(0)" }}/>
...
//       <div style={{ position:"absolute",bottom:-20,right:-10,fontSize:"clamp(130px,22vw,190px)",fontWeight:900,color:lang.accent,opacity:hov?.08:.032,lineHeight:1,fontFamily:"'Fira Code',monospace",pointerEvents:"none",userSelect:"none",transition:"opacity .4s, transform .6s",transform:hov?"translateY(-8px)":"none" }}>{lang.index}</div>

//       {/* horizontal data scan on hover */}
//       {hov && <div style={{ position:"absolute",top:0,left:0,right:0,bottom:0,pointerEvents:"none",overflow:"hidden",zIndex:1 }}>
//         <div style={{ position:"absolute",width:"100%",height:1,background:`linear-gradient(90deg,transparent,${lang.accent}44,transparent)`,animation:"cardScan 2.2s ease-in-out infinite" }}/>
//       </div>}
...
//         <div style={{ fontSize:"clamp(54px,11vw,90px)",fontWeight:800,letterSpacing:-4,lineHeight:.87,color:hov?lang.accent:"#c8d8f0",marginBottom:18,transition:"color .28s",textShadow:hov?`0 0 120px ${lang.accent}40, 0 0 40px ${lang.accent}20`:"none",fontFamily:"'Fira Code',monospace" }}>{lbl}</div>

//         <div style={{ fontSize:10,color:hov?lang.accent+"88":"#506070",letterSpacing:3,marginBottom:20,fontFamily:"'Fira Code',monospace",transition:"color .3s" }}>{lang.tagline}</div>

//         <div style={{ height:1,background:`linear-gradient(90deg,${lang.accent}80,${lang.accent}22,transparent)`,marginBottom:22,opacity:hov?1:.2,transition:"opacity .3s,background .3s" }}/>

//         <p style={{ fontSize:12,color:hov?"#7a8a9a":"#6a7a90",lineHeight:1.9,marginBottom:28,fontFamily:"'Fira Code',monospace",transition:"color .3s" }}>{lang.desc}</p>

//         <div style={{ display:"flex",gap:8,flexWrap:"wrap",marginBottom:28 }}>
//           {lang.meta.map(m=>(
//             <span key={m} style={{ fontSize:9,letterSpacing:2,padding:"5px 12px",border:`1px solid ${lang.accent}${hov?"44":"22"}`,color:hov?lang.accent:lang.accent+"66",background:`${lang.accent}${hov?"10":"05"}`,fontFamily:"'Fira Code',monospace",transition:"all .3s" }}>{m}</span>
//           ))}
//         </div>

//         <div style={{ marginBottom:38 }}>
//           {lang.features.map((f,i)=>(
//             <div key={f} style={{ fontSize:11,color:hov?"#8090a0":"#506070",marginBottom:9,display:"flex",alignItems:"center",gap:10,fontFamily:"'Fira Code',monospace",transform:hov?"translateX(8px)":"none",transition:`transform .4s ${i*.07}s ease, color .3s` }}>
//               <span style={{ color:lang.accent,fontSize:8,transition:"transform .3s",transform:hov?"scale(1.4)":"scale(1)" }}>▶</span>{f}
//             </div>
//           ))}
//         </div>

//         <button style={{ width:"100%",padding:"16px",cursor:"none",border:`1.5px solid ${lang.accent}${hov?"dd":"44"}`,color:hov?"#fff":lang.accent,fontFamily:"'Fira Code',monospace",fontSize:10,letterSpacing:4,background:hov?`${lang.accent}18`:"transparent",boxShadow:hov?`0 0 60px ${lang.accent}22,inset 0 0 40px ${lang.accent}08, 0 0 0 1px ${lang.accent}22`:"none",transition:"all .35s",position:"relative",overflow:"hidden" }}>
//           {hov && <div style={{ position:"absolute",inset:0,background:`linear-gradient(90deg,transparent,${lang.accent}10,transparent)`,animation:"btnShine 1.4s ease-in-out infinite" }}/>}
...
//   return (
//     <div ref={stripRef} style={{ display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",borderTop:"1px solid #1a2030",borderLeft:"1px solid #1a2030",marginBottom:52 }}>
//       {stats.map((s,i)=>(
//         <div key={s.label} className="stat-item" style={{ padding:"30px 24px",borderRight:"1px solid #1a2030",borderBottom:"1px solid #1a2030",fontFamily:"'Fira Code',monospace",position:"relative",overflow:"hidden",transition:"background .3s" }}
//           onMouseEnter={e=>e.currentTarget.style.background=`rgba(0,247,255,.02)`}
//           onMouseLeave={e=>e.currentTarget.style.background="transparent"}
//         >
//           <div style={{ position:"absolute",top:0,left:0,right:0,height:1,background:`linear-gradient(90deg,${s.accent}44,transparent)` }}/>
//           <div style={{ fontSize:9,letterSpacing:3,color:"#2a3548",marginBottom:10 }}>{s.label}</div>
//           <div style={{ fontSize:38,fontWeight:800,color:s.accent,letterSpacing:-2,textShadow:`0 0 50px ${s.accent}50` }}>
...
//       style={{ padding:"30px 22px",border:`1px solid ${hov?"#00f7ff40":"#1a2030"}`,background:hov?"rgba(0,247,255,.025)":"transparent",transition:"border-color .3s,background .3s",fontFamily:"'Fira Code',monospace",position:"relative",overflow:"hidden" }}>
//       {hov && <div style={{ position:"absolute",top:0,left:0,right:0,height:1,background:"linear-gradient(90deg,transparent,#00f7ff88,transparent)" }}/>}
//       <div style={{ fontSize:24,marginBottom:16,color:"#00f7ff",opacity:hov?1:.45,transition:"opacity .3s, transform .35s",transform:hov?"scale(1.2) translateX(3px)":"none" }}>{item.icon}</div>
//       <div style={{ fontSize:10,letterSpacing:3,color:hov?"#c8d8f0":"#506070",marginBottom:10,transition:"color .3s" }}>{item.title}</div>
//       <div style={{ fontSize:11,color:hov?"#3a4f60":"#2a3548",lineHeight:1.7,transition:"color .3s" }}>{item.desc}</div>
//     </div>
//   );
// }
...
//       op: 0.03 + Math.random() * 0.07, timer: 0,
//     }));

//     let radarAngle = 0;
//     const rings = Array.from({ length: 5 }, (_, i) => ({ r: 40, maxR: 100 + i * 55, op: 0.1, timer: i * 60 }));
...
//       hexes.forEach(h => {
//         h.pulse += h.speed;
//         const alpha = (Math.sin(h.pulse) * 0.5 + 0.5) * h.bright * 0.042;
//         ctx.beginPath();
//         for (let i = 0; i < 6; i++) {
...
//           ctx.fillStyle = `rgba(0,247,255,${s.op * ((s.chars.length - i) / s.chars.length)})`;
//           ctx.fillText(ch, s.x, s.y + i * 13);
//         });
//       });

//       radarAngle += 0.007;
//       const rcx = W * 0.84, rcy = H * 0.35, rr = Math.min(W * 0.14, H * 0.28);
//       ctx.beginPath(); ctx.arc(rcx, rcy, rr, 0, Math.PI * 2);
//       ctx.strokeStyle = "rgba(0,247,255,0.05)"; ctx.lineWidth = 0.5; ctx.stroke();
//       [0, Math.PI * 0.5, Math.PI, Math.PI * 1.5].forEach(a => {
//         ctx.beginPath(); ctx.moveTo(rcx, rcy); ctx.lineTo(rcx + Math.cos(a) * rr * 1.05, rcy + Math.sin(a) * rr * 1.05);
//         ctx.strokeStyle = "rgba(0,247,255,0.04)"; ctx.lineWidth = 0.4; ctx.stroke();
//       });
//       ctx.save(); ctx.translate(rcx, rcy); ctx.rotate(radarAngle);
//       for (let i = 0; i < 24; i++) {
//         const a = (i / 24) * (Math.PI * 0.6);
//         ctx.beginPath(); ctx.moveTo(0, 0); ctx.arc(0, 0, rr, -a, -(a + Math.PI * 0.025));
//         ctx.fillStyle = `rgba(0,247,255,${0.0012 * (24 - i)})`; ctx.fill();
//       }
//       ctx.beginPath(); ctx.moveTo(0, 0); ctx.lineTo(rr, 0);
//       ctx.strokeStyle = "rgba(0,247,255,0.28)"; ctx.lineWidth = 1.2; ctx.stroke();
//       for (let b = 0; b < 3; b++) {
//         const ba = -b * 0.22, br = rr * (0.28 + b * 0.25);
//         ctx.beginPath(); ctx.arc(Math.cos(ba) * br, Math.sin(ba) * br, 2.5 - b * 0.5, 0, Math.PI * 2);
//         ctx.fillStyle = `rgba(0,247,255,${0.6 - b * 0.15})`; ctx.fill();
//       }
//       ctx.restore();

//       rings.forEach(ring => {
//         ring.r += 0.32;
//         ring.op = Math.max(0, 0.13 * (1 - (ring.r - 40) / (ring.maxR - 40)));
//         if (ring.r >= ring.maxR) ring.r = 40;
//         ctx.beginPath(); ctx.arc(rcx, rcy, ring.r, 0, Math.PI * 2);
//         ctx.strokeStyle = `rgba(0,247,255,${ring.op})`; ctx.lineWidth = 0.6; ctx.stroke();
//       });

//       [[0,0],[W,0],[0,H],[W,H]].forEach(([bx, by]) => {
//         const sx = bx===0?1:-1, sy = by===0?1:-1;
//         ctx.strokeStyle = "rgba(0,247,255,0.07)"; ctx.lineWidth = 1;
...
//   return (
//     <section ref={hRef} style={{ width:"100%",minHeight:"100vh",padding:"80px clamp(24px,5vw,80px) 0",position:"relative",overflow:"hidden",display:"flex",flexDirection:"column",justifyContent:"center",boxSizing:"border-box" }}>
//       <HeroBG/>
...
//       {/* reticle */}
//       <div ref={reticleRef} style={{ position:"absolute",pointerEvents:"none",width:88,height:88,marginLeft:-44,marginTop:-44,zIndex:8,opacity:.25,top:0,left:0 }}>
...
//           <line x1="30" y1="30" x2="37" y2="37" stroke="#00f7ff" strokeWidth=".4" opacity=".5"/>
//           <line x1="58" y1="30" x2="51" y2="37" stroke="#00f7ff" strokeWidth=".4" opacity=".5"/>
//         </svg>
//       </div>
...
//         {/* eyebrow label */}
//         <div ref={eyeRef} style={{ display:"flex",alignItems:"center",gap:12,marginBottom:28,fontFamily:"'Fira Code',monospace",fontSize:9,letterSpacing:5,color:"#00f7ff88" }}>
//           <span style={{ width:36,height:1,background:"linear-gradient(90deg,#00f7ff,transparent)",display:"inline-block" }}/>
//           VISUOSLAYER // CASE #2026 // LANG-SELECT
//           <span style={{ width:36,height:1,background:"linear-gradient(270deg,#00f7ff,transparent)",display:"inline-block" }}/>
//         </div>

//         <h1 ref={h1Ref} style={{ lineHeight:.82,fontFamily:"'Fira Code',monospace",fontWeight:800,fontSize:"clamp(64px,14.5vw,190px)",letterSpacing:"-.05em",width:"100%" }}>
//           <span className="word" style={{ display:"block",color:"transparent",WebkitTextStroke:"1px rgba(200,216,240,.2)",lineHeight:.88 }}>BEGINNER?</span>
//           <span className="word" style={{ display:"block",color:"#00f7ff",textShadow:"0 0 160px rgba(0,247,255,.6),0 0 60px rgba(0,247,255,.4)",animation:"floatY 3.8s ease-in-out infinite",position:"relative",lineHeight:.88 }}>
//             SELECT
//             {/* animated scan line on SELECT */}
//             <span style={{ position:"absolute",left:0,top:"52%",width:"100%",height:2,background:"linear-gradient(90deg,transparent 0%,#00f7ff44 30%,#00f7ff 50%,#00f7ff44 70%,transparent 100%)",animation:"scanH 2.8s cubic-bezier(.4,0,.2,1) infinite",pointerEvents:"none",boxShadow:"0 0 18px #00f7ff" }}/>
//           </span>
//           <span className="word" style={{ display:"block",color:"#c8d8f0",lineHeight:.88 }}>LANGUAGE</span>
//         </h1>

//         {/* right column stats */}
//         <div style={{ position:"absolute",right:0,top:"50%",transform:"translateY(-50%)",display:"flex",flexDirection:"column",gap:22,fontFamily:"'Fira Code',monospace",fontSize:9,letterSpacing:2,textAlign:"right",zIndex:20 }}>
//           {[["FILES","03","#00f7ff"],["ENCRYPTED","YES","#ff3366"],["CLEARANCE","ALPHA","#39ff14"],["VERSION","4.002","#ffb347"]].map(([k,v,c])=>(
//             <div key={k}>
//               <div style={{ color:"#1a2030" }}>{k}</div>
//               <div style={{ color:c+"55",fontSize:15,fontWeight:800,letterSpacing:-1,textShadow:`0 0 20px ${c}30` }}>{v}</div>
//             </div>
//           ))}
//         </div>
//       </div>
...
//           <div style={{ width:1,minHeight:90,background:"linear-gradient(to bottom,#00f7ff,transparent)",flexShrink:0,marginTop:4,boxShadow:"0 0 10px #00f7ff66" }}/>
//           <p ref={subRef} style={{ fontSize:12,color:"#506070",letterSpacing:1,lineHeight:2,fontFamily:"'Fira Code',monospace" }}>
...
//             <br/><br/>
//             <span style={{ color:"#253040" }}>// VISUOSLAYER by SAIF — CASE #2026</span>
//           </p>
//         </div>

//         {/* status panel */}
//         <div ref={statusRef} style={{ flexShrink:0,border:"1px solid #1a2a3a",padding:"16px 22px",fontFamily:"'Fira Code',monospace",fontSize:10,lineHeight:1.9,color:"#253040",minWidth:240,background:"rgba(5,7,14,.8)",backdropFilter:"blur(8px)",flex:"0 0 auto",position:"relative",overflow:"hidden" }}>
//           <div style={{ position:"absolute",top:0,left:0,right:0,height:1,background:"linear-gradient(90deg,transparent,#00f7ff44,transparent)" }}/>
//           <div style={{ color:"#2a3548",fontSize:9,letterSpacing:3,marginBottom:10 }}>SYSTEM STATUS</div>
//           {[["KERNEL","OVERRIDE_ACTIVE","#00f7ff22"],["FILES","3 / UNLOCKED","#39ff1422"],["OPERATOR","SAIF","#c8d8f022"],["SIGNAL","99%","#00f7ff22"]].map(([k,v,c])=>(
//             <div key={k} style={{ display:"flex",justifyContent:"space-between",gap:24 }}>
//               <span style={{ color:"#1e2535" }}>{k}</span>
//               <span style={{ color:"#3a5060" }}>{v}</span>
//             </div>
//           ))}
//           <div style={{ marginTop:12,borderTop:"1px solid #1a2030",paddingTop:10,color:"#00f7ff55",fontSize:9,letterSpacing:2 }}>
//             &gt; {typed}<span style={{ animation:"bl 1s step-end infinite",color:"#00f7ff88" }}>_</span>
//           </div>
//         </div>
//       </div>

//       {/* bottom HUD row */}
//       <div ref={dividerRef} style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:52,paddingTop:18,borderTop:"1px solid #1a2535",fontFamily:"'Fira Code',monospace",fontSize:9,letterSpacing:2,color:"#1e2535",flexWrap:"wrap",gap:12,position:"relative",zIndex:10,transformOrigin:"left" }}>
//         <span>CAM_01 · LAT 23.0225°N · LONG 72.5714°E</span>
//         <div style={{ display:"flex",gap:16,alignItems:"center" }}>
//           {[0,1,2,3,4].map(i=><div key={i} style={{ width:28,height:1,background:i===2?"#00f7ff77":"#1a2030",boxShadow:i===2?"0 0 8px #00f7ff":"none" }}/>)}
//         </div>
//         <span>ALT 53M · <span style={{ color:"#00f7ff33" }}>{typed}<span style={{ animation:"bl 1s step-end infinite",color:"#00f7ff44" }}>_</span></span></span>
//       </div>

//       {/* ── SCROLL INDICATOR ── */}
//       <div ref={scrollArrow} style={{ position:"absolute",bottom:36,left:"50%",transform:"translateX(-50%)",zIndex:20,display:"flex",flexDirection:"column",alignItems:"center",gap:8,fontFamily:"'Fira Code',monospace",fontSize:8,letterSpacing:4,color:"#00f7ff66" }}>
//         <span>SCROLL</span>
//         <div style={{ width:1,height:32,background:"linear-gradient(to bottom,#00f7ff99,transparent)",boxShadow:"0 0 8px #00f7ff" }}/>
...
//       <div style={{ position:"fixed",inset:0,pointerEvents:"none",zIndex:3,background:"radial-gradient(ellipse at center,transparent 40%,rgba(5,6,14,.82) 100%)" }}/>
...
//         {/* ── ENHANCED SCROLL MARQUEE SECTION ── */}
//         <div style={{ overflow:"hidden",margin:"60px 0 0",position:"relative" }}>
//           {/* ambient top glow */}
//           <div style={{ position:"absolute",top:0,left:"50%",transform:"translateX(-50%)",width:"60%",height:1,background:"linear-gradient(90deg,transparent,#00f7ff55,transparent)",pointerEvents:"none",zIndex:2 }}/>
...
//           {/* ambient bottom glow */}
//           <div style={{ position:"absolute",bottom:0,left:"50%",transform:"translateX(-50%)",width:"60%",height:1,background:"linear-gradient(90deg,transparent,#ffb34755,transparent)",pointerEvents:"none",zIndex:2 }}/>
//         </div>

//         {/* CONTENT */}
//         <section style={{ maxWidth:1280,margin:"0 auto",padding:"80px 36px 140px" }}>
//           <Ticker/>
//           <StatsStrip/>
...
//               <div key={k} style={{ padding:"13px 24px",borderRight:i<a.length-1?"1px solid #1a2030":"none",fontFamily:"'Fira Code',monospace",fontSize:9,letterSpacing:2,background:"rgba(5,7,14,.7)",display:"flex",flexDirection:"column",gap:5,flexShrink:0 }}>
//                 <span style={{ color:"#253040" }}>{k}</span>
//                 <span style={{ color:c,textShadow:`0 0 16px ${c}44` }}>{v}</span>
//               </div>
//             ))}
//           </div>
...
//           <div ref={warnRef} style={{ display:"flex",alignItems:"center",gap:14,padding:"14px 22px",border:"1px solid #ff336630",background:"rgba(255,51,102,.025)",fontFamily:"'Fira Code',monospace",fontSize:9,color:"#2a3548",letterSpacing:2,position:"relative",overflow:"hidden" }}>
//             <div style={{ position:"absolute",top:0,left:0,right:0,height:1,background:"linear-gradient(90deg,#ff336600,#ff336655,#ff336600)" }}/>
...
//             <span style={{ color:"#1e2535",marginLeft:"auto",flexShrink:0 }}> // VISUOSLAYER OS — AUTHORISED BY SAIF</span>
//           </div>
//         </section>

//         <footer style={{ borderTop:"1px solid #1a2030",maxWidth:1280,margin:"0 auto",padding:"22px 36px",display:"flex",justifyContent:"space-between",alignItems:"center",fontFamily:"'Fira Code',monospace",fontSize:9,color:"#253040",letterSpacing:2,flexWrap:"wrap",gap:10 }}>
//           <span>VISUOSLAYER // SAIF</span>
//           <span>LANG_FILES — CASE #2026</span>
//           <span>ALL FILES ENCRYPTED — AUTHORISED PERSONNEL ONLY</span>
//           <span style={{ color:"#00f7ff22" }}>SYS.STABLE</span>
//         </footer>
//       </main>
...
//         @keyframes bl{0%,100%{opacity:1}50%{opacity:0}}
//         ::selection{background:#00f7ff12;color:#00f7ff}
...
function HUD({ activeLang }) {
  ...
  return (
    <div ref={ref} style={{ position:"fixed",top:0,left:0,right:0,zIndex:100,display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 32px",background:"rgba(5,6,14,.92)",borderBottom:"1px solid #1a2030",backdropFilter:"blur(20px)",fontFamily:"'Fira Code',monospace",fontSize:10,letterSpacing:2,transition:"background 0.2s, backdrop-filter 0.2s" }}>
      <div style={{ display:"flex",gap:24,alignItems:"center" }}>
        <div style={{ display:"flex",alignItems:"center",gap:8 }}>
          <span style={{ width:6,height:6,borderRadius:"50%",background:"#00f7ff",display:"block",boxShadow:"0 0 10px #00f7ff",animation:"hp 2.5s ease-in-out infinite" }}/>
          <span style={{ color:"#c8d8f0",fontWeight:800,letterSpacing:4 }}>VISUOSLAYER</span>
...
// ─── PREMIUM PROFESSIONAL TECH GLOBE (Replaces old orb) ─────────────────────
...
function PremiumGSAPOrb() {
  ...
  useEffect(() => {
    ...
    // Helper: draw sphere wireframe (latitude/longitude)
    function drawSphere(ctx, centerX, centerY, radius, rotX, rotY, rotZ, color, lineWidth) {
      const stepsLat = 16;  // latitude lines (rings)
      const stepsLong = 24; // longitude lines
      ...
      for (let i = 1; i < stepsLat; i++) {
        const phi = (i / stepsLat) * Math.PI; // 0 to PI
    ...
    const draw = () => {
      ...
      // Dynamic color based on scroll (premium gradient)
      const hue = 200 + progress * 100; // blue to purple range
      const mainColor = `hsla(${hue}, 80%, 65%, 0.9)`;
      const accentColor = `hsla(${hue + 30}, 85%, 70%, 0.85)`;
      const glowColor = `hsla(${hue}, 85%, 65%, 0.3)`;
      ...
      ctx.scale(1, 0.95); // subtle vertical squash for perspective
      ctx.translate(-centerX, -centerY);
      
      // Rotate sphere dynamically
      const rotX = t * 0.2;
      const rotY = t * 0.15;
      const rotZ = t * 0.05;
      drawSphere(ctx, centerX, centerY, baseRadius, rotX, rotY, rotZ, mainColor, 1.2);
      
      // Inner sphere highlight (glow)
      ctx.shadowBlur = 15;
      ctx.shadowColor = `hsla(${hue}, 85%, 70%, 0.8)`;
      drawSphere(ctx, centerX, centerY, baseRadius * 0.98, rotX, rotY, rotZ, accentColor, 0.8);
      ctx.shadowBlur = 0;
      
      ctx.restore();
      
      // Orbiting rings (ellipses for 3D effect)
      const ringCount = 3;
      ...
      for (let i = 0; i < particleCount; i++) {
        ...
        const size = 1.8 + Math.sin(t * 5 + i) * 0.8;
        ctx.beginPath();
      ...
      // Floating data points (subtle)
      for (let i = 0; i < 50; i++) {
        const angle = i * 0.8 + t * 0.9;
        const rad = baseRadius * (0.65 + Math.sin(i) * 0.2);
        const x = centerX + Math.cos(angle) * rad;
        const y = centerY + Math.sin(angle * 1.3) * rad * 0.8;
        ctx.beginPath();
        ctx.arc(x, y, 1, 0, Math.PI * 2);
        ctx.fillStyle = `hsla(${hue + i * 4}, 80%, 70%, 0.5)`;
        ctx.fill();
      }
      
      // Mouse interaction: subtle highlight
  ...
  useEffect(() => {
    ...
    const st = ScrollTrigger.create({
      ...
      onUpdate: self => {
        ...
        // Update line color based on scroll progress
        if (line) {
...
function Card({ lang, onSelect, index }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ position:"absolute",bottom:-20,right:-10,fontSize:"clamp(130px,22vw,190px)",fontWeight:900,color:lang.accent,opacity:hov?.1:.04,lineHeight:1,fontFamily:"'Fira Code',monospace",pointerEvents:"none",userSelect:"none",transition:"opacity .4s, transform .6s",transform:hov?"translateY(-8px)":"none" }}>{lang.index}</div>
      ...
      <div style={{ position:"relative",zIndex:2 }}>
        ...
        <div style={{ fontSize:"clamp(54px,11vw,90px)",fontWeight:800,letterSpacing:-4,lineHeight:.87,color:hov?lang.accent:"#c8d8f0",marginBottom:18,transition:"color .28s",textShadow:hov?`0 0 120px ${lang.accent}60, 0 0 40px ${lang.accent}30`:"none",fontFamily:"'Fira Code',monospace" }}>{lbl}</div>
        ...
        <p style={{ fontSize:12,color:hov?"#8a9ab0":"#6a7a90",lineHeight:1.9,marginBottom:28,fontFamily:"'Fira Code',monospace",transition:"color .3s" }}>{lang.desc}</p>
        ...
        <button style={{ width:"100%",padding:"16px",cursor:"none",border:`1.5px solid ${lang.accent}${hov?"ff":"66"}`,color:hov?"#fff":lang.accent,fontFamily:"'Fira Code',monospace",fontSize:10,letterSpacing:4,background:hov?`${lang.accent}25`:"transparent",boxShadow:hov?`0 0 70px ${lang.accent}33,inset 0 0 50px ${lang.accent}15, 0 0 0 1px ${lang.accent}44`:"none",transition:"all .35s",position:"relative",overflow:"hidden" }}>
    ...
    <div ref={stripRef} style={{ display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",borderTop:"2px solid rgba(0,247,255,0.25)",borderLeft:"1px solid #1a2030",marginBottom:52,transition:"box-shadow 0.2s" }}>
      {stats.map((s,i)=>(
        ...
        >
          ...
          <div style={{ fontSize:9,letterSpacing:3,color:"#2a3548",marginBottom:10 }}>{s.label}</div>
          <div style={{ fontSize:38,fontWeight:800,color:s.accent,letterSpacing:-2,textShadow:`0 0 60px ${s.accent}80` }}>
            <AnimCounter to={s.val} suffix={s.suffix}/>
          </div>
        </div>
      ))}
...
function FeatureItem({ item, idx }) {
  ...
  return (
    <div ref={ref} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      style={{ padding:"30px 22px",border:`1px solid ${hov?"#00f7ff60":"#1a2030"}`,background:hov?"rgba(0,247,255,.04)":"transparent",transition:"border-color .3s,background .3s",fontFamily:"'Fira Code',monospace",position:"relative",overflow:"hidden" }}>
      ...
      <div style={{ fontSize:24,marginBottom:16,color:"#00f7ff",opacity:hov?1:.55,transition:"opacity .3s, transform .35s",transform:hov?"scale(1.2) translateX(3px)":"none" }}>{item.icon}</div>
      <div style={{ fontSize:10,letterSpacing:3,color:hov?"#c8d8f0":"#506070",marginBottom:10,transition:"color .3s" }}>{item.title}</div>
      <div style={{ fontSize:11,color:hov?"#4a6f80":"#2a3548",lineHeight:1.7,transition:"color .3s" }}>{item.desc}</div>
    </div>
  );
}
...
function Hero({ eyeRef, h1Ref, subRef }) {
  ...
  return (
    <section ref={hRef} style={{ width:"100%",minHeight:"100vh",padding:"80px clamp(24px,5vw,80px) 0",position:"relative",overflow:"hidden",display:"flex",flexDirection:"column",justifyContent:"center",boxSizing:"border-box" }}>
      <HeroBG/>
      <PremiumGSAPOrb /> {/* Replaced with professional tech globe */}
      ...
      <div style={{ position:"relative",zIndex:10 }}>
        <div ref={eyeRef} style={{ display:"flex",alignItems:"center",gap:12,marginBottom:28,fontFamily:"'Fira Code',monospace",fontSize:9,letterSpacing:5,color:"#00f7ffaa" }}>
      ...
      <div style={{ display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:40,marginTop:56,position:"relative",zIndex:10,flexWrap:"wrap" }}>
        <div style={{ display:"flex",gap:24,alignItems:"flex-start",maxWidth:520,flex:"1 1 300px" }}>
          ...
          <p ref={subRef} style={{ fontSize:12,color:"#607080",letterSpacing:1,lineHeight:2,fontFamily:"'Fira Code',monospace" }}>
            Three encrypted files. Each contains the fundamentals of a programming language.
            Select one to begin decryption. Your mission starts now.
            <br/><br/>
        ...
        <div ref={statusRef} style={{ flexShrink:0,border:"1px solid #2a3a4a",padding:"16px 22px",fontFamily:"'Fira Code',monospace",fontSize:10,lineHeight:1.9,color:"#2a4050",minWidth:240,background:"rgba(5,7,14,.85)",backdropFilter:"blur(12px)",flex:"0 0 auto",position:"relative",overflow:"hidden" }}>
    ...
    <>
      ...
      <main style={{ position:"relative",zIndex:10,paddingTop:80,opacity:vis?1:0,transition:"opacity 1s cubic-bezier(.4,0,.2,1)" }}>
        ...
        <section style={{ maxWidth:1280,margin:"0 auto",padding:"80px 36px 140px" }}>
          ...
          <div ref={warnRef} style={{ display:"flex",alignItems:"center",gap:14,padding:"14px 22px",border:"1px solid #ff3366aa",background:"rgba(255,51,102,.035)",fontFamily:"'Fira Code',monospace",fontSize:9,color:"#3a5060",letterSpacing:2,position:"relative",overflow:"hidden" }}>
        ...
        <footer style={{ borderTop:"2px solid rgba(0,247,255,0.3)",maxWidth:1280,margin:"0 auto",padding:"22px 36px",display:"flex",justifyContent:"space-between",alignItems:"center",fontFamily:"'Fira Code',monospace",fontSize:9,color:"#2a4050",letterSpacing:2,flexWrap:"wrap",gap:10 }}>
          <span>VISUOSLAYER // SAIF</span>
          <span>LANG_FILES — CASE #2026</span>
          <span>ALL FILES ENCRYPTED — AUTHORISED PERSONNEL ONLY</span>
